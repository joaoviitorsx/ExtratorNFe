...
def folderCard(folder_name: str, total_files: int, on_change_folder, on_next):
    ...
    return ft.Column([
        ft.Row([
            ft.Row([
                ft.Icon(ft.Icons.FOLDER_OPEN, color=th["ICON"], size=20),
...
def completedCard(total_files: int, validos: int, cancelados: int, erros: int, lista_erros: list, on_download, on_new_folder, lista_cancelados: list = None):
    ...
    # Seção para arquivos cancelados
    if lista_cancelados:
        cancelados_visiveis = lista_cancelados[:3]
        mais_cancelados = len(lista_cancelados) - 3 if len(lista_cancelados) > 3 else 0
        lista_cancelados_text = [ft.Text(f"• {os.path.basename(nome)}", size=12) for nome, _ in cancelados_visiveis]
        if mais_cancelados > 0:
            lista_cancelados_text.append(
                ft.Text(f"... e mais {mais_cancelados} arquivo(s) cancelado(s).", size=12, italic=True, color=th["TEXT_SECONDARY"])
            )
        card_content.insert(-1, ft.Container(
            content=ft.Column([
                ft.Row([
                    ft.Icon(ft.Icons.CANCEL, color=ft.Colors.ORANGE_700, size=16),
                    ft.Text("Arquivos cancelados:", weight=ft.FontWeight.W_500, size=14)
                ], spacing=8),
                ft.Column(lista_cancelados_text, spacing=2)
            ], spacing=8),
            bgcolor=th["CARD"],
    ...
    # Seção para arquivos com erro
    if lista_erros:
        erros_visiveis = lista_erros[:3]
        mais_erros = len(lista_erros) - 3 if len(lista_erros) > 3 else 0
        lista_erros_text = [ft.Text(f"• {os.path.basename(nome)}", size=12)for nome, _ in erros_visiveis]
        if mais_erros > 0:
